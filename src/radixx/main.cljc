(ns radixx.main
  "Kotodama WASM entrypoint for the Radixx clean-room actor (L4) — Clojure port.
  ...
  facts (`radixx.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "radixx")
(def tier "L4")
...
(def entity-specs
  [{:entity "Flight"      :plural "flights"      :id-prefix "radixx_fli"
    :fields [:carrier :number :origin :destination :status] :required [:carrier :number]
    :coerce {}                       :refs {}}
   {:entity "Booking"     :plural "bookings"     :id-prefix "radixx_boo"
    :fields [:pnr :passengerName :status]         :required [:pnr :passengerName]
    :coerce {}                       :refs {}}
   {:entity "Segment"     :plural "segments"     :id-prefix "radixx_seg"
    :fields [:bookingId :flightId :cabin :seat]  :required [:cabin :seat]
    :coerce {}                       :refs {:bookingId "Booking" :flightId "Flight"}}
   {:entity "Property"    :plural "properties"   :id-prefix "radixx_pro"
    :fields [:name :location :rooms]             :required [:name :location]
    :coerce {:rooms :int}            :refs {}}
   {:entity "Reservation" :plural "reservations" :id-prefix "radixx_res"
    :fields [:propertyId :guestName :checkIn :nights] :required [:guestName :checkIn]
    :coerce {:nights :int}           :refs {:propertyId "Property"}}
   {:entity "Fare"        :plural "fares"        :id-prefix "radixx_far"
    :fields [:flightId :cabin :price :currency]  :required [:cabin :price]
    :coerce {:price :float}          :refs {:flightId "Flight"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"radixx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "radixx-compat" :tier tier :entities entities} 200])
...
(defn start! [] :radixx-compat/ready)
